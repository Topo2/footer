...
function TotalSoftGalleryV_Edit_Option(Gallery_Video_OptID)
{
	...
	jQuery.post(ajaxurl, data, function(response) {
		...
		{ 
			b[b.length]=a[i].split('[')[0].trim(); 
			b_name[b_name.length]=a[i].split('[')[1];
		}
		b[b.length-1]=b[b.length-1].split(')')[0].trim();		
		...
		setTimeout(function(){
			...
			function TotalSoft_Options(){
				for(i=2;i<=b_name.length-2;i++)
				{
					var res_array=b_name[i].split(']');
					var c=res_array[0];
					if(jQuery('#'+c).attr('type')=='checkbox')
					{
						if(b[i+1]=='true' || b[i+1]==1)
						{
							jQuery('#'+c).attr('checked',true)
						}
						else if(b[i+1]=='false' || b[i+1]=='')
						{
					...
					{
						if(c=='TotalSoft_VG_GVG_HO')
						{
							jQuery('#'+c).val(parseInt(parseFloat(b[i+1])*100));
						}
						else
						{
							jQuery('#'+c).val(b[i+1]);
